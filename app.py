...
with st.expander("📖 **Как пользоваться калькулятором** — нажми чтобы открыть инструкцию", expanded=False):
    ...
    with tab4:
        ...
        - Ты сам доставляешь покупателю
        - Самая низкая комиссия
        
        #### 🎯 Какую выбрать в калькуляторе?
        - **FBS** → выбирай "📦 FBS (Маркетплейс)"
        - **FBO** → выбирай "🏬 FBO (Склад WB)"
        - **Разные** → выбирай "🔀 Смешанная (авто)"
        """)
    ...
    with tab5:
        ...
        **🛡️ Плавно снижать** (рекомендуется)
        - Если у тебя скидка 60%, снижаем по 5% за раз
        - Безопасно от карантина
        
        **🎯 Установить сразу**
        - Ставит нужную скидку сразу
        - Риск карантина при резком снижении
        
        **🔒 Не менять**
        - Оставляет скидку как есть
        - Может дать гигантскую "перечёркнутую цену"
    ...
    with tab6:
        ...
        а НЕ реальную прибыль твоего магазина.
        ...
        - **Маржу** — процент прибыли от цены продажи
        - **Все расходы** — комиссия, логистика, налоги, себестоимость
        ...
        Все расходы: −1 118 ₽
        ─────────────────
        Прибыль с 1 продажи: 282 ₽ ✅
        ```
        
        Это значит: **если продашь этот товар — заработаешь 282₽ с одной единицы**.
        
        ---
        
        #### ❌ Что мы НЕ считаем
        
        - **Сколько ты реально продал** — это не наши данные
        - **Общую прибыль за месяц** — зависит от объёма продаж
        - **Расходы на рекламу** — не учитываем
        - **Возвраты клиентов** — учитываем только среднее (по % выкупа)
        ...
        Чтобы понять:
        - ✅ Какие товары **прибыльные** (можно продвигать)
        - 🔴 Какие товары **убыточные** (нужно поднять цену)
        - 💰 Какая **оптимальная цена** для целевой маржи
        - 📊 Куда уходят деньги (структура расходов)
        
        ---
        
        #### 🎯 Реальная прибыль ≠ Расчётная
        
        **Пример:**
        - Товар A: прибыль с 1 продажи 500₽, продано 100 шт = 50 000₽/мес
        - Товар B: прибыль с 1 продажи 800₽, продано 5 шт = 4 000₽/мес
        
        **Товар A выгоднее**, хотя маржа ниже!
        
        Для полной картины нужны данные о фактических продажах 
        (это уже другой инструмент — дашборд аналитики).
        """)
    ...
    with tab7:
        ...
        **Q: Это моя реальная прибыль?**  
        A: **Нет!** Это гипотетическая прибыль с ОДНОЙ продажи. Реальная зависит от того, сколько ты продашь.
        
        **Q: Почему рекомендованная цена высокая?**  
        A: Учитываем **все реальные расходы**. Если работал в убыток — цена вырастет.
        
        **Q: % выкупа — где посмотреть?**  
        A: В личном кабинете WB → Аналитика → Заказы и продажи.
        
        ---
        
        #### 🏬 Про модели
        
        **Q: Некоторые товары FBS, некоторые FBO. Что выбрать?**  
        A: Выбирай "🔀 Смешанная (авто)".
        
        ---
        
        #### 💰 Про обновление цен
        
        **Q: Что происходит при обновлении цен?**  
        A: Калькулятор отправляет новые цены в WB через API. Появятся через 5-15 минут.
        
        **Q: Можно ли откатить изменения?**  
        A: Только вручную. Мы работаем над функцией отката.
        ...
        **Q: "WB ограничил запросы" — что делать?**  
        A: Подожди 1-2 минуты и попробуй снова.
        
        **Q: Что если калькулятор упал?**  
        A: Обнови страницу (Cmd+Shift+R). Данные сохранены в кеше.
        """)
...
<div class="disclaimer-box">
    ...
    <a href="#" style="color: #92400E; text-decoration: underline;">Подробнее в инструкции ↑</a>
</div>
""", unsafe_allow_html=True)
...
with col_g3:
    ...
    st.caption("💡 Гипотетическая прибыль если продашь ОДНУ единицу этого товара")
...
with col_g4:
    st.markdown("##### 🚨 ТОП-10 самых убыточных с 1 продажи")
    st.caption("💡 Сколько теряешь на каждой ЕДИНИЦЕ этого товара")
