...
with st.sidebar:
    ...
    # НОВЫЙ БЛОК: настройки возвратов
    with st.expander("🔄 Настройки возвратов (продвинуто)"):
        ...
        return_processing_fee = st.number_input(
            ...
            help="Плата WB за обработку одного возврата (приёмка, проверка, размещение)"
        )
        
        damage_rate_percent = st.slider(
            "% порчи товара при возврате",
            min_value=0, max_value=50, value=10, step=1,
            help="Какой % возвращённых товаров приходит в непродаваемом виде (списание)"
        )
        ...
        💡 **Рекомендации:**
        - Одежда/обувь: 10% порчи
        - Аксессуары/косметика: 5% порчи
        - Хрупкие товары: 15-20% порчи
        """)
...
with st.expander("📖 **Как пользоваться калькулятором** — нажми чтобы открыть инструкцию", expanded=False):
    ...
    with tab6:
        ...
        ✅ Комиссию WB (правильную для модели)  
        ✅ Логистику (по объёму товара)  
        ✅ Эквайринг 1.5%  
        ✅ Налоги (УСН/ОСНО)  
        ...
        ```
        Товар A123, цена 1400₽, выкуп 90%
        
        Комиссия WB 22%: -308₽
        Логистика:       -65₽
        Эквайринг:       -21₽
        Возвраты:        -14₽ (полный расчёт)
          ├─ Логистика:  6.5₽
          ├─ Обработка:  3₽
          └─ Порча:      5₽
        Налог УСН 6%:   -84₽
        ...
        - Товар A: 500₽/шт × 100 продаж = 50 000₽/мес
        - Товар B: 800₽/шт × 5 продаж = 4 000₽/мес
    ...
    with tab7:
        ...
        **Q: Как правильно указать % порчи товара?**  
        A: Одежда — 10%, аксессуары — 5%, хрупкое — 15-20%.
        
        **Q: Обработка возврата — сколько?**  
        A: Обычно 30₽. Если крупногабарит — до 100₽.
        
        **Q: "WB ограничил запросы" — что делать?**  
        A: Подожди 1-2 минуты, попробуй снова.
        
        **Q: Что если калькулятор упал?**  
        A: Обнови страницу (Cmd+Shift+R). Данные в кеше.
        """)
...
with st.expander("🔄 Детализация расходов на возвраты"):
    if "return_logistics_rub" in df_results.columns:
        ...
        st.caption(f"""
        💡 **Расчёт:** при выкупе {buyout_percent}% на каждую продажу приходится 
        {(100-buyout_percent)/100:.0%} возвратов. Полная стоимость одного возврата = 
        логистика + {return_processing_fee}₽ обработка + {damage_rate_percent}% порчи.
        """)
...
with col_exp3:
    json_export = {
        ...
        "settings": {
            ...
            "damage_rate_percent": damage_rate_percent,
        },
        "summary": {
